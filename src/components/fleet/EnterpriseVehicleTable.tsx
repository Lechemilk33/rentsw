...
interface EnterpriseVehicleTableProps {
  ...
  onLocationView: (vehicle: Vehicle) => void
  onGasCalculatorOpen: (vehicle: Vehicle) => void
  isLoading?: boolean
...
export const EnterpriseVehicleTable: React.FC<EnterpriseVehicleTableProps> = ({
  ...
  const columns = [
    ...
    {
      key: 'wash_last_updated' as const,
      label: 'Last Updated',
      width: 'w-32',
      sortable: true
    },
    ...
    {
      key: 'gas_calculator' as const,
      label: 'Gas Calculator',
      width: 'w-32',
      sortable: false
    },
    {
      key: 'location_view' as const,
      label: 'Location',
      width: 'w-24',
      sortable: false
    }
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-600">
                      {vehicle.wash_last_updated ? (
                        <div>
                          <div className="font-medium">
                            {new Date(vehicle.wash_last_updated).toLocaleDateString()}
                          </div>
                          <div className="text-xs text-gray-500">
                            {new Date(vehicle.wash_last_updated).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </div>
                        </div>
                      ) : (
                        <span className="text-gray-400 italic">Never washed</span>
                      )}
                    </div>
                  </td>
                  ...
                  {/* Gas Calculator Column */}
                  <td className="px-6 py-4">
                    <button
                      onClick={() => onGasCalculatorOpen(vehicle)}
                      className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                      aria-label={`Open gas calculator for ${vehicle.make} ${vehicle.model}`}
                    >
                  ...
                  <td className="px-6 py-4">
                    <button
                      onClick={() => onLocationView(vehicle)}
                      className="p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                      aria-label={`View location for ${vehicle.make} ${vehicle.model}`}
                    >
